 ...
 */
public class EditRingingTaskActivity extends Base2Activity {
    ...
    private void initView() {
        ...
        if (TaskUtils.getIsManager()) {
            ...
            ToastUtil.show(EditRingingTaskActivity.this, "温馨提示：普通用户不能编辑信息！");
        }
    ...
    public void complete() {
        ...
        for (Task task1 : taskList) {
            if (task1.getTaskNum() != task.getTaskNum()) {
                data.add(task1);
            }
        }

        if (!TaskUtils.checkTaskTime(data, task)) {
            checkDate();
            return;
        }
