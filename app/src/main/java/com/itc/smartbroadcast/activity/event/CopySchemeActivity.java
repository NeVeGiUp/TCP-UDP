...
public class CopySchemeActivity extends Base2Activity {
    private ImageView btBackEvent;
    private TextView tvAddringover, tvStartDate, tvEndDate, tvTitle;
    ...
    private void initData() throws Exception {
        ...
        String schemeJson = intent.getStringExtra("scheme");
        String taskListJson = intent.getStringExtra("taskList");
        taskList = JSONArray.parseArray(taskListJson, Task.class);
        Gson gson = new Gson();
        scheme = gson.fromJson(schemeJson, Scheme.class);
        etTaskName.setText(scheme.getSchemeName() + "克隆");
        mSchemeStartTime = scheme.getSchemeStartDate();
        mSchemeEndTime = scheme.getSchemeEndDate();
        Date startDate = sf2.parse(mSchemeStartTime);
        Date endDate = sf2.parse(mSchemeEndTime);
        mStartTime = sf.format(startDate);
        mEndTime = sf.format(endDate);
    ...
    private void initView() {
        ...
        tvTitle = (TextView) findViewById(R.id.tv_edit_scheme_title);
        btEndDate = (RelativeLayout) findViewById(R.id.bt_enddate);

        tvTitle.setText("克隆方案");
    ...
    private void addRingingScheme() {
        ...
        EditScheme.sendCMD(AppDataCache.getInstance().getString("loginIp"), scheme, 3);
    }
        ...
        if ("editSchemeResult".equals(baseBean.getType())) {
            ...
            if (data != null) {
                ...
                if (editSchemeResult.getResult() == 1) {
                    ToastUtil.show(this, "方案克隆成功!");
                    this.finish();
                } else {
                    ToastUtil.show(this, "方案克隆失败，方案数目达到最大值，请删除部分方案重试!");
                }
            } else {
            ...
            switch (view.getId()) {
                ...
                case R.id.tv_addringover:
                    ...
                    if (!StringUtils.checkName(mTaskName)) {
                        ToastUtil.show(CopySchemeActivity.this, "方案名称必须是汉字、字母和数字并且在2到15个字符之间！", Toast.LENGTH_SHORT);
                        break;
                    }
                    if (mSchemeStartTime == null || mSchemeStartTime.equals("")) {
                        ToastUtil.show(CopySchemeActivity.this, "开始时间不能为空！", Toast.LENGTH_SHORT);
                        break;
                    }
                    if (mSchemeEndTime == null || mSchemeEndTime.equals("")) {
                        ToastUtil.show(CopySchemeActivity.this, "结束时间不能为空！", Toast.LENGTH_SHORT);
                        break;
                    }
                    ...
                    try {
                        ...
                        if (endDate.getTime() < startDate.getTime()) {
                            ToastUtil.show(CopySchemeActivity.this, "结束时间不能小于开始时间！", Toast.LENGTH_SHORT);
                            break;
                        }
                        ...
                        if (startDate.getTime() < now.getTime()) {
                            ToastUtil.show(CopySchemeActivity.this, "方案开始时间必须为今天以及以后的时间！", Toast.LENGTH_SHORT);
                            break;
                        }
