...
public class EditPartitionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StatusBarUtil.setColor(EditPartitionActivity.this, getResources().getColor(R.color.colorMain), 0);
    ...
    private void init() {
        ...
        accId = Integer.parseInt(intent.getStringExtra("AccId"));
        partNum = intent.getStringExtra("PartNum");
        ...
        for (int i = 0; i < deviceList.size(); i++) {
            FoundDeviceInfo deviceInfo = deviceList.get(i);

            ArrayList<Integer> zoneMsg = deviceInfo.getDeviceZoneMsg();
            int index = 0;
            for (int j = 0; j < zoneMsg.size(); j++) {
                if (zoneMsg.get(j) == Integer.parseInt(partNum)) {
                    index = j;
                }
            }
            deviceInfo.setDeviceZone(deviceInfo.getDeviceZoneFlagMsg().get(index));
            deviceList.set(i, deviceInfo);
        }
        ...
        btBackEvent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (checkPoint == 1){
                    ...
                    v = View.inflate(EditPartitionActivity.this, R.layout.dialog_tips, null);
...
//                EditPartitionActivity.this.startActivityForResult(intent, 3);
        ...
        tvSavePart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (partName.length() < 2 || CheckEmojiUtils.containsEmoji(partName)) {
                    ToastUtil.show(EditPartitionActivity.this, "请输入正确的分区名");
                } else {
                    ...
                    partition.setPartitionNum(Integer.parseInt(partNum));
                    partition.setPartitionName(partName);
                    Log.i("选择的分区编号》》》",partName);
                    ...
                    if (checkedMac.equals("")) {
                        ToastUtil.show(EditPartitionActivity.this, "请绑定终端");
                    } else if (partName.equals("")) {
                        ToastUtil.show(EditPartitionActivity.this, "请输入终端名");
                    } else {
                        EditPartition.sendCMD(AppDataCache.getInstance().getString("loginIp"), partition, 2);
                        try {
                            Thread.sleep(300);
    ...
    public void onMessageEventMain(String json) {
        ...
        if ("editPartitionResult".equals(baseBean.getType())) {
            ...
            if (operator.equals("00")) {
                if (result == 0) {
                    ToastUtil.show(EditPartitionActivity.this, "新建失败");
                } else {
                    ToastUtil.show(EditPartitionActivity.this, "新建成功");
                }
            }
            if (operator.equals("01")) {
                if (result == 0) {
                    ToastUtil.show(EditPartitionActivity.this, "删除失败");
                } else {
                    init();
                    ToastUtil.show(EditPartitionActivity.this, "删除成功");
                }
            }
            if (operator.equals("02")) {
                if (result == 0) {
                    ToastUtil.show(EditPartitionActivity.this, "修改失败");
                } else {
                    ToastUtil.show(EditPartitionActivity.this, "修改成功");
                }
            }
    ...
    public void getDevice() {
        Intent intent = new Intent(EditPartitionActivity.this, ChooseAmplifierTerminalActivity.class);
        Gson gson = new Gson();
