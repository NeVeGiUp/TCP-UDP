...
public class EditSchemeActivity extends Base2Activity {
    ...
    private void initData() throws Exception {
        ...
        etTaskName.setText(scheme.getSchemeName());
    ...
    private void addRingingScheme() {
        ...
        if (!StringUtils.checkName(mTaskName)) {
            ToastUtil.show(EditSchemeActivity.this, "方案名称必须是汉字、字母和数字并且在2到15个字符之间！", Toast.LENGTH_SHORT);
            return;
        }
        if (mSchemeStartTime == null || mSchemeStartTime.equals("")) {
            ToastUtil.show(EditSchemeActivity.this, "开始时间不能为空！", Toast.LENGTH_SHORT);
            return;
        }
        if (mSchemeEndTime == null || mSchemeEndTime.equals("")) {
            ToastUtil.show(EditSchemeActivity.this, "结束时间不能为空！", Toast.LENGTH_SHORT);
            return;
        }
        ...
        try {
            ...
            if (endDate.getTime() < startDate.getTime()) {
                ToastUtil.show(EditSchemeActivity.this, "结束时间不能小于开始时间！", Toast.LENGTH_SHORT);
                return;
            }
            ...
            if (startDate.getTime() < now.getTime()) {
                ToastUtil.show(EditSchemeActivity.this, "方案开始时间必须为今天以及以后的时间！", Toast.LENGTH_SHORT);
                return;
            }
