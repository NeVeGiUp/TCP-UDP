...
import com.itc.smartbroadcast.channels.protocolhandler.ActivationRegisterCode;
...
public class NettyUdpClientHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    public static List<byte[]> buffList = new ArrayList<>();
    ...
    protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket msg)
        ...
        int writerIndex = msg.content().writerIndex();  //获取有效字节长度,总长度
        byte[] array = msg.content().array();           //缓冲区里面得所有数据
        byte[] alldata = SmartBroadCastUtils.subBytes(array, 0, writerIndex);  //从缓冲区里截取有效数据

        Log.e(NettyUdpClient.TAG, "消息来源" + msg.sender().getHostString() + ":" + msg.sender().getPort());
        Log.e(NettyUdpClient.TAG, "接收服务器数据包 >>> " + SmartBroadCastUtils.bytesToHexString(alldata));
    ...
    private void dvGlobalControl(int packetSize, int packetIndex, ChannelHandlerContext ctx, String hexCommandType, List<byte[]> buffList) {
        ...
        switch (hexCommandType) {
            case "02":  //定时器主动推送消息更新列表
                ListRefresh.init().handler(buffList);
                buffList.clear();
                SmartBroadcastApplication.replyData = true;
                break;
            case "04":  //获取终端设备状态协议
                ...
                break;
            case "0c":  //搜索局域网内设备
                SearchDeviceList.init().handler(buffList);
                buffList.clear();
                SmartBroadcastApplication.replyData = true;
                break;
            case "0a":  //配置目标主机
                ConfigureTargetHost.init().handler(buffList);
                buffList.clear();
                SmartBroadcastApplication.replyData = true;
                break;
            default:
    ...
    private void dvAccountManageType(int packetSize, int packetIndex, ChannelHandlerContext ctx, String hexCommandType, List<byte[]> buffList) {
        switch (hexCommandType) {
            ...
            case "05":  //定时器在线状态查询
                ...
                break;
            case "0d":  //客户端激活注册机器协议
                ActivationRegisterCode.init().handler(buffList);
                buffList.clear();
                SmartBroadcastApplication.replyData = true;
                break;
            default:
    ...
    private void dvDeviceType(int packetSize, int packetIndex, ChannelHandlerContext ctx, String hexCommandType, List<byte[]> buffList) {
        switch (hexCommandType) {
            ...
            case "08":  //查询控制面板详情
                ...
                break;
            case "09":  //配置控制面板绑定终端数
                EditControlPanel.init().handler(buffList);
                buffList.clear();
                SmartBroadcastApplication.replyData = true;
                break;
            case "0a":  //查询采集器音源类型
...
//                SmartBroadcastApplication.replyData = true;
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg == null || msg == Unpooled.EMPTY_BUFFER || msg instanceof EmptyByteBuf) {
            // TODO: 2018/8/6 判断接收不到数据执行重发机制 处理丢包问题（UDP）
            Log.e(NettyUdpClient.TAG, "channelRead接收数据为空，准备重发 ");
        } else {
        }
        super.channelRead(ctx, msg);   //这里调用得是channelRead0（）
    }
