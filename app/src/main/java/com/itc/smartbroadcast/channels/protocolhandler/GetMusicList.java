...
public class GetMusicList {
    ...
    public static GetMusicList init() {
    ...
    private List<MusicMsgInfo> byteToFoundDeviceInfo(byte[] b) {
        ...
        //文件夹名称
        String musicFolderName = SmartBroadCastUtils.byteToStr(SmartBroadCastUtils.subBytes(b, HEAD_PACKET_LENGTH + 3, FOLDER_NAME_LENGTH));
        //一个item所占字节数
        int itemBt = MUSIC_NAME_LENGTH + MUSIC_TIME_LENGTH;
        byte[] musicListBt = SmartBroadCastUtils.subBytes(b, FOLDER_NAME_LENGTH + HEAD_PACKET_LENGTH + 3, itemBt * musicCount);
        for (int i = 0; i < musicListBt.length; i += itemBt) {
            ...
            musicInfo.setMusicFolderName(musicFolderName);
    ...
    //发送指令 需要传递文件夹名称
    public static void sendCMD(String host, String musicFolder) {
        //判断是否为云协议
        if (SmartBroadcastApplication.isCloud) {
            //判断TCP连接是否连接上
            if (NettyTcpClient.isConnSucc) {
                byte[] bytes = SmartBroadCastUtils.CloudUtil(getMusicList(musicFolder), host, false);
                NettyTcpClient.getInstance().sendPackage(host, bytes);
            }
        } else {
            NettyUdpClient.getInstance().sendPackage(host,getMusicList(musicFolder));
        }
    }
    ...
    public static byte[] getMusicList(String musicFolder) {
        String hexMusicFolder = SmartBroadCastUtils.chinese2Hex(musicFolder, GetMusicList.FOLDER_NAME_LENGTH);
        String allHexMusicHolder = SmartBroadCastUtils.str2HexStr(musicFolder);
        int count = GetMusicList.FOLDER_NAME_LENGTH * 2;
